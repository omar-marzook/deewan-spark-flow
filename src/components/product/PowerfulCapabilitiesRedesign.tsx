                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isActive 
                        ...
                        : "bg-white/40 border-white/20 hover:bg-white/60 hover:shadow-md"
                    )}
                  >
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        ...
                        isActive 
                          ? "bg-gradient-to-br from-deewan-primary to-deewan-primary/70 text-white" 
                          : "bg-white/70 text-deewan-primary group-hover:bg-white/90"
                      )}>
                        {React.createElement(feature.icon, { className: "h-5 w-5" })}
                      </div>
                      <span className={cn(
                        "font-medium transition-colors duration-300",
                        isActive ? "text-deewan-dark" : "text-deewan-dark/70"
                      )}>
                        {feature.title}
                      </span>
                    </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center p-8">
                {features.map(feature => {
                  ...
                  return (
                    <motion.div
                      key={feature.id}
                      className="absolute inset-0 flex flex-col justify-between h-full w-full p-8"
                        ...
                        <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-white/20 shadow-lg relative">
                          <div className="flex items-center">
                            ...
                            <p className="text-sm text-deewan-dark/80 italic">
                              "{feature.id === 'ai' ? 'Reduce response time by up to 80%' : 
                                feature.id === 'analytics' ? 'Understand customer behavior in real-time' :
                                feature.id === 'realtime' ? 'Deliver messages in milliseconds' :
                                feature.id === 'centralized' ? 'Manage all communications from one place' :
                                feature.id === 'api' ? 'Connect with any channel or service' :
